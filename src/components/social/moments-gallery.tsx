...
interface Moment {
  ...
  likes: string | number;
  aspect?: string;
  embedLink?: string; // For TikTok/Ig embeds if needed
}
...
const CATEGORIES = ['All', 'Mabar', 'Tournament', 'Drilling', 'Fun Match', 'Merch', 'TikTok'];

export default function MomentsGallery({ initialMoments = [] }: MomentsGalleryProps) {
  // Merge initial moments with static ones if desired, or just use initial
  // For now, let's append some static placeholders if initial is empty or just mix them

  // Fallback Mock Data (if unconnected)
  const MOCK_MOMENTS: Moment[] = [
    ...
    // ... can add more mocks if needed
  ];

  const [moments] = useState<Moment[]>(initialMoments.length > 0 ? [...initialMoments, ...MOCK_MOMENTS] : MOCK_MOMENTS);
  const [activeCategory, setActiveCategory] = useState('All');
  const [selectedItem, setSelectedItem] = useState<Moment | null>(null);
    ...
    <div>
      ...
      <div className="relative px-6 mb-16">
        ...
        <div className="container mx-auto text-center relative z-10">
          ...
          <p className="text-slate-500 font-medium max-w-2xl mx-auto text-xl mb-10 leading-relaxed">
            ...
            {initialMoments.length > 0 && <span className="block text-sm font-bold text-[#ca1f3d] mt-2">✨ Live synced with TikTok</span>}
          </p>
            ...
            >
              <div className={cn(
                ...
                <div className="absolute bottom-0 left-0 w-full p-8 translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                  ...
                  <h3 className="text-2xl font-black text-white leading-tight mb-2 drop-shadow-md line-clamp-2">{item.title}</h3>
      ...
      <Dialog open={!!selectedItem} onOpenChange={(open) => !open && setSelectedItem(null)}>
        <DialogContent className="max-w-6xl w-full h-[95vh] md:h-[85vh] bg-white p-0 overflow-hidden rounded-[3rem] flex flex-col md:flex-row gap-0 shadow-2xl border-0">
          ...
          <div className="relative flex-1 bg-black flex items-center justify-center h-[50vh] md:h-full group/video">
            {selectedItem && (
              selectedItem.type === 'video' && selectedItem.embedLink ? (
                <iframe
                  src={`${selectedItem.embedLink}?autoplay=1`}
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#ffbe00] to-[#ff0099] flex items-center justify-center text-xl font-black text-white shadow-lg">
                    {selectedItem?.user?.charAt(0) || 'B'}
                  </div>
                  <div>
                    <h4 className="font-black text-slate-900 text-xl">{selectedItem?.user || 'Badmintour'}</h4>
                    <p className="text-sm font-bold text-slate-400">Official Post</p>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-6">
                  <button className="flex items-center gap-2 group">
                    ...
                    <span className="text-base font-bold text-slate-700">{selectedItem?.likes || 0}</span>
                  </button>
                  <button className="flex items-center gap-2 group">
