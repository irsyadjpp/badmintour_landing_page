...
export default function CoachSidebar() {
    ...
    const navItems = [
        ...
        { href: "/coach/drills", icon: Dumbbell, label: "Modul Latihan" },
                        ...
                        {navItems.map((item) => {
                            ...
                            return (
                                <Tooltip key={item.label}>
                                    <TooltipTrigger asChild>
                                        <Link 
                                            ...
                                            className={cn(
                                                ...
                                                isActive 
                                                    ? "bg-[#00f2ea] text-black shadow-[0_0_20px_rgba(0,242,234,0.5)] scale-110" 
                                    ...
                                    <TooltipContent side="right" className="ml-4 bg-[#1A1A1A] text-[#00f2ea] border-[#00f2ea]/20 text-xs font-black px-3 py-1.5 rounded-lg shadow-xl uppercase tracking-wider">
                                        {item.label}
                                    </TooltipContent>
                                </Tooltip>
                            );
                        ...
                        return (
                            <Link key={item.label} href={item.href} className="flex flex-col items-center gap-1">
                                <div className={cn(
                                    ...
                                    isActive ? "bg-[#00f2ea] text-black shadow-lg shadow-[#00f2ea]/30" : "text-gray-500"
                                )}>
                                    <item.icon className="w-5 h-5" />
                                </div>
                                <span className={cn("text-[9px] font-bold", isActive ? "text-[#00f2ea]" : "text-gray-600")}>
                                    {item.label.split(' ')[0]}
                                </span>
                            </Link>
                        );
