...
export default function HostScanPage() {
    // --- STATE ---
    const [scanResult, setScanResult] = useState<any>(null); // null, 'success', 'error'
    ...
    const handleScan = (data: any) => {
        if (data && !isProcessing) {
            ...
            setTimeout(() => {
                // Contoh: Kode valid jika diawali "TIC-"
                const isValid = resultText.startsWith("TIC-");
    ...
    const handleManualScan = (data: string | null) => {
         if (data && !isProcessing) {
            setIsProcessing(true);
            setScannedData(data);
        ...
        <div className="space-y-6 pb-24 max-w-md mx-auto relative min-h-[80vh]">
            ...
            <Tabs defaultValue="camera" className="w-full">
                ...
                <TabsContent value="camera" className="relative mt-0">
                    <div className="relative aspect-square bg-black rounded-[2rem] overflow-hidden border-2 border-dashed border-[#333] flex items-center justify-center group">
                            ...
                            <>
                                ...
                                <div className="w-full h-full">
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                        <div className="w-48 h-48 border-2 border-[#ca1f3d] rounded-2xl relative">
                                            <div className="absolute top-0 left-0 w-full h-1 bg-[#ca1f3d] shadow-[0_0_15px_#ca1f3d] animate-scan-down"></div>
                                        </div>
                                    </div>
                                </div>

                                {/* DEBUG BUTTON (Untuk Testing di Laptop tanpa Kamera) */}
                                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                                     <Button size="sm" variant="secondary" onClick={() => handleScan({ text: "TIC-12345" })} className="opacity-50 hover:opacity-100 text-[10px]">
                                        Test Valid
                                     </Button>
                                     <Button size="sm" variant="destructive" onClick={() => handleScan({ text: "INVALID-CODE" })} className="opacity-50 hover:opacity-100 text-[10px]">
                                        Test Fail
                                     </Button>
                                </div>
                            </>
                             ...
                             <div className="flex flex-col items-center justify-center p-6 text-center animate-in zoom-in duration-300">
                                <p className="text-gray-400 text-xs mb-2 animate-pulse">Processing...</p>
                            </div>
                        )}
