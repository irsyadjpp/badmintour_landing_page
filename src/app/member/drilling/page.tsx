...
import { Loader2, MapPin, Calendar, Clock, Dumbbell, User } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen pt-24 px-6 pb-12 max-w-lg mx-auto">
      <Card className="bg-[#151515] border-white/10 p-8 rounded-[2rem] space-y-6 relative overflow-hidden">
        {/* Visual Header Cyan untuk Drilling */}
        <div className="absolute top-0 left-0 right-0 h-2 bg-[#00f2ea]"></div>
        ...
        <div className="text-center">
          ...
          {/* Harga */}
          <p className="font-bold mt-2 text-lg text-[#00f2ea]">
            Rp {event.price.toLocaleString('id-ID')}
          </p>
        </div>

        {/* Coach Info (Wajib ada di Drilling) */}
        {event.coachName && (
        ...
        <div className="space-y-4 bg-black/20 p-6 rounded-2xl border border-white/5">
          <div className="flex items-center gap-3 text-gray-300">
            <Calendar className="w-5 h-5 text-[#00f2ea]" />
            <span>{new Date(event.date).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long' })}</span>
          </div>
          <div className="flex items-center gap-3 text-gray-300">
            <Clock className="w-5 h-5 text-[#00f2ea]" />
            <span>{event.time}</span>
          </div>
          <div className="flex items-center gap-3 text-gray-300">
            <MapPin className="w-5 h-5 text-[#00f2ea]" />
            <span>{event.location}</span>
          </div>
          <div className="flex items-center gap-3 text-gray-300">
            <User className="w-5 h-5 text-[#00f2ea]" />
        ...
        {!session && (
          <div className="bg-[#1A1A1A] p-4 rounded-xl border border-[#00f2ea]/30 space-y-4 animate-in fade-in slide-in-from-bottom-2">
            <div className="flex items-center gap-2 mb-2">
              <span className="w-2 h-2 rounded-full bg-[#00f2ea] animate-pulse"></span>
              <p className="text-xs font-bold text-[#00f2ea] uppercase tracking-widest">Guest Mode (Tanpa Login)</p>
            </div>
        ...
        <Button
          ...
          className="w-full h-14 font-black text-xl rounded-xl shadow-lg bg-[#00f2ea] text-black hover:bg-[#00c2bb]"
        >
