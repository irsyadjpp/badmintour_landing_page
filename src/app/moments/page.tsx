                ...
                <Dialog open={!!selectedItem} onOpenChange={(open) => !open && setSelectedItem(null)}>
                    <DialogContent className="max-w-5xl w-full h-[90vh] md:h-auto bg-[#0a0a0a]/95 backdrop-blur-xl border-white/10 p-0 overflow-hidden rounded-[2.5rem] flex flex-col md:flex-row gap-0">
                        <div className="relative flex-1 bg-black flex items-center justify-center h-1/2 md:h-auto">
                            ...
                            <Button onClick={() => setSelectedItem(null)} className="absolute top-4 right-4 md:left-4 rounded-full w-10 h-10 bg-black/50 hover:bg-black/80 text-white p-0 border border-white/10 z-50">
                                <X className="w-5 h-5" />
                            </Button>
                        </div>
                        <div className="w-full md:w-[400px] bg-[#151515] p-6 md:p-8 flex flex-col justify-between h-1/2 md:h-auto border-l border-white/5 overflow-y-auto">
                            <div>
                                ...
                                <Badge className="mb-4 bg-[#00f2ea]/10 text-[#00f2ea] border-[#00f2ea]/20">{selectedItem?.category}</Badge>
                                <h2 className="text-2xl md:text-3xl font-black text-white italic leading-tight mb-4">{selectedItem?.title}</h2>
                                <p className="text-gray-400 text-sm leading-relaxed">Momen seru dari kegiatan {selectedItem?.category}. Bagikan keseruan ini ke teman-temanmu!</p>
                            </div>
                            <div className="space-y-4 mt-6">
                                <div className="flex items-center justify-between py-4 border-t border-white/5">
                                    <div className="flex items-center gap-4">
                                        <button className="flex items-center gap-2 group">
                                            <div className="p-2 rounded-full bg-white/5 group-hover:bg-[#ff0099]/20 transition-colors"><Heart className="w-5 h-5 text-gray-400 group-hover:text-[#ff0099]" /></div>
                                            <span className="text-sm font-bold text-white">{selectedItem?.likes}</span>
                                        </button>
                                        <button className="flex items-center gap-2 group">
                                            <div className="p-2 rounded-full bg-white/5 group-hover:bg-[#00f2ea]/20 transition-colors"><Share2 className="w-5 h-5 text-gray-400 group-hover:text-[#00f2ea]" /></div>
                                            <span className="text-sm font-bold text-white">Share</span>
                                        </button>
                                    </div>
                                </div>
                                <Button className="w-full h-14 bg-white text-black hover:bg-gray-200 font-bold rounded-xl shadow-lg">Lihat Profil {selectedItem?.user}</Button>
                            </div>
                        </div>
                    </DialogContent>
                </Dialog>
